...
//static uint8_t OLED_GRAM_turn[8][128];
...
void oled_showchar1(uint8_t row, uint8_t col, int chr)
{
    uint8_t x = col * 1;
    uint8_t y = row * 1;
    uint8_t temp, t, t1;
    ...
    for (t = 0; t < 80; t++)
    {
        ...
        {
            if (temp&0x80)
                oled_drawpoint(y, x, Pen_Write);
            else
                oled_drawpoint(y, x, Pen_Clear);
...
void oled_number(uint8_t row, uint8_t col, int a, uint8_t mode, uint8_t len)
{
			if(a<0)
			{
				oled_printf(row,col,"-");	
				oled_shownum(row,col+1,-a,mode,len);
			}	
			else
			oled_shownum(row,col+1,a,mode,len);
}

//void oled_number_int(uint8_t row, uint8_t col, uint32_t a, uint8_t mode, uint8_t len)
//{
//			if(a<0)
//			{
//				oled_printf(row,col,"-");	
...
//			oled_shownum(row,col+1,a,mode,len);
//}
//void oled_small_number(uint8_t row, uint8_t col, float *a 
//											 ,int *integer ,int *decimal 
//											 ,uint8_t integer_num ,uint8_t decimal_num)
//											 //整数为integer，小数为decimal，小数点前integer_num位，小数点后decimal_num位
//{
//	int point = 0;
//	point = col + integer_num;
//	pid_change_in(*a,integer,decimal,decimal_num);	
////		oled_number(row,col-1,*integer,0,integer_num);
//	oled_shownum(row,col,*integer,0,integer_num);	
//	oled_printf(row,point,".");
//	oled_shownum(row,point+1,*decimal,1,decimal_num);
//}

#ifdef imu_dataread_test
//void oled_small_number(uint8_t row, uint8_t col, float *a 
//											 ,int *integer ,int *decimal 
//											 ,uint8_t integer_num ,uint8_t decimal_num)
//											 //整数为integer，小数为decimal，小数点前integer_num位，小数点后decimal_num位
...
////	uint32_t b= -9999;
//	point = col + integer_num;
//	small_number(*a,integer,decimal,decimal_num);	
//	oled_number(row,col-1,*integer,0,integer_num);
////	oled_shownum(row,col,*integer,0,integer_num);	
//	oled_printf(row,point,".");
//	oled_shownum(row,point+1,*decimal,1,decimal_num);
...
#else
void oled_small_number(uint8_t row, uint8_t col, float *a ,uint8_t integer_num ,uint8_t decimal_num)
											 //整数为integer，小数为decimal，小数点前integer_num位，小数点后decimal_num位
{
	int point = 0;
	int integer;
